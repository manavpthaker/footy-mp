...
/* ---------- app header (mobile root + desktop with inline tabs) ---------- */
function AppHeader({ right = null, tabs = false, active, onSelect }) {
  return (
    <header style={{
      position: "sticky", top: 0, zIndex: 20, background: "var(--grad-header)",
      borderBottom: "1px solid var(--border)", padding: tabs ? "0 20px" : "0 16px",
    }}>
      <div className={tabs ? undefined : "fmp-cap"} style={{
        display: "flex", alignItems: "center", gap: 9,
        ...(tabs
          ? { maxWidth: 1280, margin: "0 auto", minHeight: "var(--header-h)" }
          : { padding: "10px 0 9px" }),
      }}>
        <div style={{
        ...
        {tabs && <HeaderTabs active={active} onSelect={onSelect} />}
...
function HeaderTabs({ active, onSelect }) {
  return (
    <nav style={{ display: "flex", alignSelf: "stretch", marginLeft: 14 }}>
      {TABS.map(t => {
        const on = t.id === active;
        return (
          <button key={t.id} className="fmp-htab" data-on={on ? "true" : "false"}
            ...
            <span style={{ fontSize: 13, lineHeight: 1, filter: on ? "none" : "grayscale(1)" }}>{t.glyph}</span>
            {t.label}
            <span style={{
...
function TabBar({ active, onSelect, showLabels = true }) {
  return (
    <nav style={{
      ...
      <div className="fmp-cap" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)" }}>
      {TABS.map(t => {
        const on = t.id === active;
        return (
...
Object.assign(window, {
  AppHeader, HeaderTabs, ScreenHeader, TabBar, FixtureItem, DayGroupedFixtures,
